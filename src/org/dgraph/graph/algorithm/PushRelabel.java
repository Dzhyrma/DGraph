...
public class PushRelabel {
	...
	public static <V, E extends FlowEdge<V>> double getMaximumFlow(FlowNetwork<V, E> network, V source, V sink, boolean resetFlowOnStart) {
					...
					if (e.getFlow() > 0) {
						...
						if (vLabel < label.get(t).intValue()) {
							...
							if (e.getFlow() - vExcess > -error) {
								excess.put(v, Double.valueOf(0d));
								excess.put(t, excess.get(t).doubleValue() + vExcess);
								e.setFlow(Math.abs(e.getFlow() - vExcess) < error ? 0d : e.getFlow() - vExcess);
								continue active;
							} else {
								excess.put(t, excess.get(t).doubleValue() + diff);
								excess.put(v, vExcess - diff);
