...
public class FibonacciHeap<T> {
    ...
    public static class Node<T> {
        ...
        private final FibonacciHeap<T> creator;
    ...
    public Node<T> enqueue(T value, double priority) {
        ...
        size++;
        final Node<T> newNode = new Node<>(value, priority, this);
        if (min == null) {
            min = newNode;
    ...
    public Node<T> dequeueMin() {
        ...
        if (min != null) {
            @SuppressWarnings("rawtypes") final Node[] visited = new Node[32 - Integer.numberOfLeadingZeros(size)];
